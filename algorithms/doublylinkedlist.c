...
void swap(struct node* ref1, struct node* ref2) {
	...
	if((iref1+1)==iref2){
		...
		vnode->data = ref1->data;
		vnode->prev = prev(ref1);
		vnode->next = next(ref1);
		//printf("Vnode %d  %d  %d \n", prev(vnode)->data, vnode->data, next(vnode)->data);
		//printf("next node %d \n",next(next(vnode))->data);
		...
		ref2->prev = prev(vnode);
		ref2->next = next(vnode);
		
		free(vnode);
		//printf("--ref2-- %d  %d  %d \n", prev(ref2)->data, ref2->data, next(ref2)->data);
	}
}
